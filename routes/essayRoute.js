...
(function() {
  var Grades, createAndAdd, populateDB;

  Grades = require('../schemas/schemas').EssayGrades;

  Grades.count({}, function(err, c) {
    if (err) {
      console.log(err);
    }
    if (c === 0) {
  ...
  exports.findAllEssays = function(req, res) {
    return Grades.find(function(err, items) {
  ...
  exports.getByIdAuthor = function(req, res) {
    var id;
    id = req.route.params['Author'];
    return Grades.findById(id, function(err, result) {
